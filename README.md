...
모든 데이터(문서, 시뮬레이션 결과, 리서치 등)는 **사용자의 로컬 브라우저(localStorage)**에 저장됩니다. 서버에 개인 데이터가 영구 보관되지 않으며, API 키 또한 브라우저에만 암호화되어 저장됩니다.

## 🤝 함께 만들어요

이 프로젝트는 아직 초기 단계입니다. 더 나은 시뮬레이션, 더 풍부한 분석, 더 다양한 활용 사례를 함께 만들어갈 분을 찾고 있습니다. 아이디어, 버그 리포트, PR 모두 환영합니다. 같이 진화시켜 봅시다!
